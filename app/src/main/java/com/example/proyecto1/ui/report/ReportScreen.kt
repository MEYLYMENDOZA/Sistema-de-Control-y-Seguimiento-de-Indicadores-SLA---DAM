...
// ESTA ES LA PANTALLA DEL ANTIGUO DASHBOARD, AHORA REUBICADA COMO "REPORTES"
@Composable
fun ReportScreen(viewModel: GestionDatosViewModel) {
    ...
    LazyColumn(
        ...
        item { ComplianceByRoleChart(uiState.records) }
    }
}

@Composable
private fun ReportSummaryCard(total: Int, compliant: Int, nonCompliant: Int) {
...
@Composable
private fun KpiCard(value: String, label: String, backgroundColor: Color = Color.LightGray.copy(alpha = 0.2f)) {
...
@Composable
private fun ComplianceByRoleChart(records: List<SlaRecord>) {
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            if (dataByRole.isEmpty()) {
                ...
                AndroidView(
                    ...
                    update = { chart ->
                        val entries = dataByRole.values.mapIndexed { index, value -> BarEntry(index.toFloat(), value) }
                        val labels = dataByRole.keys.toList()
                        val dataSet = BarDataSet(entries, "Cumplimiento por Rol").apply { color = barColor }
